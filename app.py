...
if "memoria_aprendizaje" not in st.session_state:
    st.session_state.memoria_aprendizaje = {}  # Aquí guardará lo que tú le enseñes hoy
...
def buscar_fichas(consulta, historial):
    ...
    # Creamos términos normalizados (ej: "cm202" de una lista de palabras)
    ...
    for item in base_conocimiento:
        ...
        # Aplicación de candados de exclusión por zona
        ...
        for p, p_norm in zip(palabras, palabras_normalizadas):
            # Valida coincidencia exacta o normalizada (ej: cm202 encuentra cm-202)
            if p in texto_base or p_norm in texto_normalizado:
                puntos += 5
            if p_norm in nombre_normalizado:
                puntos += 40 # Súper bono si el producto viene en el nombre del archivo
                
        # Máxima prioridad si encuentra tu archivo personalizado de la tienda
...
# 6. Caja de Retroalimentación y Aprendizaje en Vivo (Punto 3)
    ...
    with st.expander("🎓 ¡Profesor FesterParedes! Enséñale la respuesta correcta a la IA", expanded=True):
        ...
        if st.button("Guardar lección en la memoria de la IA"):
            if nueva_respuesta.strip():
                # Guardamos la palabra clave normalizada en la memoria en vivo
                ...
                st.success("¡Entendido! He aprendido la lección. Si vuelven a preguntar esto, responderé exactamente tus palabras.")
                st.session_state.mostrar_caja_retro = False
                st.rerun()

# 7. Ejecución de la entrada del usuario
if prompt := st.chat_input("¿Qué problema tienes en obra o qué producto deseas validar?"):
    ...
    # REVISIÓN EN TU MEMORIA EN VIVO (Si ya le enseñaste la respuesta antes)
    respuesta_aprendida = ""
    ...
    if not contexto_manuales.strip():
        # Si no encontró nada en los PDFs, activa automáticamente la caja de aprendizaje
        with st.chat_message("assistant"):
...
3. Si el usuario te da m² o cantidad de charolas, calcula el consumo basándote en la ficha inferior.
...
"""
    ...
    with st.chat_message("assistant"):
        try:
            ...
            # Si la IA responde el mensaje de "No tengo información", activamos la retroalimentación
