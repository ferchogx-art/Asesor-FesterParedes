...
# Modelo estable de alta velocidad y precisión para el manual de mostrador
...
def buscar_fichas(consulta, historial):
    ...
    contexto_final = ""
    # CORRECCIÓN DE KEY DE ORDENAMIENTO USANDO EL ÍNDICE NUMÉRICO [0] EN AMBAS LISTAS
    ...
    if any(g in prompt_lower for g in ["gotera", "filtracion", "tiene goteras", "tengo filtraciones", "ya tiene"]):
        if "azotea" in historial_texto or "techo" in historial_texto or "losa" in historial_texto:
            respuesta_directa = (
                ...
                "4. Refuerzo en puntos críticos (bajantes, chaflanes) con malla Acriflex or Revoflex.<br/>"
...
"""
    ...
    with st.chat_message("assistant"):
        try:
            ...
            # EXTRACCIÓN SINTÁCTICA MAESTRA COMPATIBLE CON EL NUEVO MOTOR LLAMA
